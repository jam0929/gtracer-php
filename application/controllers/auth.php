...
class Auth extends CI_Controller
{
    ...
    function __construct()
    {
        ...
        $this->lang->load('tank_auth', $this->config->item('language'));
    }
    ...
    function login()
    {
        if ($this->tank_auth->is_logged_in()) { // logged in
            ...
            $data['login_by_username'] 
                = ($this->config->item('login_by_username', 'tank_auth') 
                AND $this->config->item('use_username', 'tank_auth'));
            $data['login_by_email'] 
            ...
            $data['use_recaptcha'] 
                = $this->config->item('use_recaptcha', 'tank_auth');
    ...
    function register()
    {
        if ($this->tank_auth->is_logged_in() && !isset($_GET['redirect_to'])) { // logged in
            redirect(base_url());
            ...
            if ($use_username) {
                $this->form_validation->set_rules(
                    ...
                    .']|max_length['
                    .$this->config->item('username_max_length', 'tank_auth')
                    .']|alpha_dash'
                );
            }
            $this->form_validation->set_rules(
                'email', 'Email', 'trim|required|xss_clean|valid_email'
            );
            $this->form_validation->set_rules(
                'password', 
                'Password', 
                'trim|required|xss_clean|min_length['
                .$this->config->item('password_min_length', 'tank_auth')
                .']|max_length['
            ...
            $captcha_registration    
                = $this->config->item('captcha_registration', 'tank_auth');
            $use_recaptcha            
                = $this->config->item('use_recaptcha', 'tank_auth');
            ...
            if ($this->form_validation->run()) {    // validation ok
                if (!is_null(
                    ...
                    if(isset($_GET['redirect_to'])) {
                        redirect(base_url($_GET['redirect_to']));
                    }
                    ...
                    if ($email_activation) {    // send "activate" email
                        $data['activation_period'] = $this->config->item(
                            'email_activation_expire', 'tank_auth'
                        ) / 3600;
                        ...
                        if ($this->config->item(
                            'email_account_details', 'tank_auth'
                        )) {    // send "welcome" email
    ...
    function forgot_password()
    {
        if ($this->tank_auth->is_logged_in()) { // logged in
            ...
            $this->form_validation->set_rules(
                'login', 'Email or login', 'trim|required|xss_clean'
            );
    ...
    function change_password()
    {
        if (!$this->tank_auth->is_logged_in()) {    // not logged in or not activated
            ...
            $this->form_validation->set_rules(
                'old_password', 'Old Password', 'trim|required|xss_clean'
            );
            $this->form_validation->set_rules(
                'new_password', 
                'New Password', 
    ...
    function _create_captcha()
    {
        ...
        $cap = create_captcha(
            array(
                'img_path'  
                    => './'.$this->config->item('captcha_path', 'tank_auth'),
                'img_url'   
                    => base_url().$this->config->item('captcha_path', 'tank_auth'),
                'font_path' 
                    => './'.$this->config->item('captcha_fonts_path', 'tank_auth'),
                'font_size' 
                    => $this->config->item('captcha_font_size', 'tank_auth'),
                'img_width' 
                    => $this->config->item('captcha_width', 'tank_auth'),
                'img_height'    
                    => $this->config->item('captcha_height', 'tank_auth'),
                'show_grid' 
                    => $this->config->item('captcha_grid', 'tank_auth'),
                'expiration'    
                    => $this->config->item('captcha_expire', 'tank_auth'),
            )
        );
    ...
    function _check_captcha($code)
    {
        ...
        } elseif (($this->config->item('captcha_case_sensitive', 'tank_auth')
            AND $code != $word)
    ...
    function _create_recaptcha()
    {
        ...
        $html = recaptcha_get_html(
            $this->config->item('recaptcha_public_key', 'tank_auth')
        );
